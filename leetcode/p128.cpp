...
void p128::buildback_BFSDFS(const string &beginWord,
...
// BFS with quque
// Solution A:
vector<vector<string>> p128::findLadders_BFSDFS(string beginWord, string endWord, vector<string>& wordList) {
...
void p128::build_forward(const string & begin, const string & end,
	vector<string> &tmp,
	const string cur,
...
vector<vector<string>> p128::findLadders_BFSDFS_sets(string beginWord, string endWord, vector<string>& wordList) {
	vector<vector<string>> ans;
	unordered_set<string> dict{ wordList.begin(), wordList.end() };
	if (!dict.count(endWord)) return ans;
	vector<string> tmp;
...
// Solution C:
vector<vector<string>> p128::findLadders(string beginWord, string endWord, vector<string>& wordList) {
	vector<vector<string>> ans;
	unordered_set<string> dict{ wordList.begin(), wordList.end() };
	if (!dict.count(endWord)) return ans;
	vector<string> tmp;
